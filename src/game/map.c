        ...
        for (int xx = 0; xx < w; xx++) {
            ...
            switch (type) {
                ...
                case TILE_WATER: {
                    ...
                    g = 100.0f + 40.0f * gy;
                    b = 180.0f + 50.0f * gy;
                    r *= 1.0f + wave; g *= 1.0f + wave; b *= 1.0f + wave;
                } break;
                case TILE_SAND: {
                ...
                case TILE_ROCK: {
                    ...
                    r *= 1.0f + rough; g *= 1.0f + rough; b *= 1.0f + rough;
                } break;
                case TILE_ROAD_NS:
                case TILE_ROAD_EW:
                case TILE_ROAD_CROSS: {
                    ...
                    g = 92.0f * (1.0f + rough);
                    b = 96.0f * (1.0f + rough);
                    if (type != TILE_ROAD_EW && ((xx - x_min) % 20) < 2) { r = 230.0f; g = 210.0f; b = 80.0f; }
                    if (type != TILE_ROAD_NS && (yy % 20) < 2) { r = 230.0f; g = 210.0f; b = 80.0f; }
                } break;
                case TILE_SHADOW: {
                    r = 0.0f; g = 0.0f; b = 0.0f; a = 120.0f;
                } break;
                default: {
                    r *= 1.0f + n; g *= 1.0f + n; b *= 1.0f + n;
                } break;
            }
...
void map_render(Map* map, SDL_Renderer* renderer, const Camera* camera) {
    ...
    for (int y = 0; y < map->height; y++) {
        for (int x = 0; x < map->width; x++) {
            ...
            if (screen_pos.x < -100 || screen_pos.x > camera->size.x + 100 ||
                screen_pos.y < -100 || screen_pos.y > camera->size.y + 100) {
                continue;
            }

            SDL_Rect src_rect = {0, 0, ISO_TILE_WIDTH, ISO_TILE_HEIGHT};
            SDL_Rect dst_rect = {
                (int)screen_pos.x - ISO_TILE_WIDTH / 2,
