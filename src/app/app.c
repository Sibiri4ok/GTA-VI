...
App* app_create(const char* title, int width, int height) {
    ...
    app->player_texture = IMG_LoadTexture(
...
void app_destroy(App* app) {
    ...
    if (app->player_texture_down) {
        SDL_DestroyTexture(app->player_texture_down);
    }
    if (app->player_texture_forward) {
        SDL_DestroyTexture(app->player_texture_forward);
    }
...
void app_update(App* app) {
    ...
    if (tile_x < 0 || tile_y < 0 || tile_x_right >= app->map->width || tile_y_top >= app->map->height) {
